...
describe('<Toast /> structural tests', () => {
  ...
  it('should render children', () => {
    ...
    const ToastComponent = shallow(<Toast><p>{testString}</p></Toast>);
    expect(ToastComponent.find('p').text()).toEqual(testString);
  });
  ...
  it('should render DANGER style if type is NETWORK_ERROR', () => {
    const ToastComponent = shallow(<Toast type={Type.NETWORK_ERROR}>Success</Toast>);
    expect(ToastComponent.find('.Toast.Danger')).toHaveLength(1);
  });
});
